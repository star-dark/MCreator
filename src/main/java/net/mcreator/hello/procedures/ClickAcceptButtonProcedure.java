...
public class ClickAcceptButtonProcedure {
	public static void execute(Entity entity) {
		if (entity == null)
			return;
		if (entity.getData(HelloModVariables.PLAYER_VARIABLES).MainQuestRequest == true) {
			if (entity.getData(HelloModVariables.PLAYER_VARIABLES).MainQuestExist == false) {
		...
		} else if (entity.getData(HelloModVariables.PLAYER_VARIABLES).SubQuestRequest == true) {
			if (entity.getData(HelloModVariables.PLAYER_VARIABLES).SubQuestExist == false) {
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal("\uD018\uC2A4\uD2B8\uB97C \uC218\uB77D\uD588\uC2B5\uB2C8\uB2E4!"), false);
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal(entity.getData(HelloModVariables.PLAYER_VARIABLES).quest_Contents), false);
