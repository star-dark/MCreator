...
public class QuestGuiScreen extends AbstractContainerScreen<QuestGuiMenu> {
    ...
    QuestProvider1Procedure.execute(world)

 instanceof LivingEntity livingEntity) {
				this.renderEntityInInventoryFollowsAngle(guiGraphics, this.leftPos + 194, this.topPos + 61, 30,
					0f ,
					0,
					livingEntity
				);
	...
	@Override protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
			guiGraphics.drawString(this.font,
				Component.translatable("gui.hello.quest_gui.label_kweseuteu"),
				80, 16, -12829636, false);
				if (        

    IsQuest2Procedure.execute(entity)

)
			guiGraphics.drawString(this.font,
				

    Quest2Procedure.execute(entity)
,
				20, 91, -12829636, false);
				if (        

    IsComplete1Procedure.execute(entity)

)
			guiGraphics.drawString(this.font,
				Component.translatable("gui.hello.quest_gui.label_wanryo"),
				142, 38, -16711936, false);
				if (        

    IsComplete2Procedure.execute(entity)

)
			guiGraphics.drawString(this.font,
				Component.translatable("gui.hello.quest_gui.label_wanryo1"),
				142, 93, -16711936, false);
				if (        

    IsQuest1Procedure.execute(entity)

)
			guiGraphics.drawString(this.font,
				

    Quest1Procedure.execute(entity)
,
				20, 38, -12829636, false);
			guiGraphics.drawString(this.font,
				Component.translatable("gui.hello.quest_gui.label_bosang"),
				31, 59, -12829636, false);
			guiGraphics.drawString(this.font,
				

    BringRewardProcedure.execute(entity)
,
				65, 59, -12829636, false);
			guiGraphics.drawString(this.font,
				Component.translatable("gui.hello.quest_gui.label_bosang1"),
				30, 110, -12829636, false);
			guiGraphics.drawString(this.font,
				

    BringRewardProcedure.execute(entity)
,
				65, 110, -12829636, false);
	}
	...
	@Override public void init() {
		super.init();
			...
			imagebutton_money_tap = new ImageButton(
				...
				32, 32,
				new WidgetSprites(ResourceLocation.parse("hello:textures/screens/money_tap.png"), ResourceLocation.parse("hello:textures/screens/money_tapon.png")),
e -> {
		if (        true
) {
			...
			imagebutton_quest_tap = new ImageButton(
				...
				32, 32,
				new WidgetSprites(ResourceLocation.parse("hello:textures/screens/quest_tap_on.png"), ResourceLocation.parse("hello:textures/screens/quest_tap_on.png")),
e -> {
}
			) {
				@Override public void renderWidget(GuiGraphics guiGraphics, int x, int y, float partialTicks) {
